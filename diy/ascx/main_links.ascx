...
<script runat="server">
    ...
    string xmlFileCache = "SAlinksXml";
    string xmlSortBy = "Name";
    string entryName = "entry";
    string[] elements = new string[] { "Name", "Url", "Description" };
    ...
    void buildTable()
    {
        ...
        while (iterator.MoveNext())
        {
            ...
            for (int a = 0; a < elements.Length; a++)
            {
                ...
                if (elements[a] == "Url")
                    cell.InnerHtml += "<a href='" + node.SelectSingleNode(elements[a]).InnerText + "' target='_blank'>" + node.SelectSingleNode(elements[a]).InnerText + "</a>";
                else if (elements[a] == "Name")
                    cell.InnerHtml += "<nobr>" + node.SelectSingleNode(elements[a]).InnerText + "</nobr>";
