...
<script runat="server">
    ...
    void Page_Load(object sender, EventArgs e)
    {
		...
		encoders   = saDevices.getDevicesByDeviceType("Encoder");
...
<table width="100%" cellpadding="2" cellspacing="0" border="0">
    <tr style="background-color:#fbfbfb">
        ...
        <td valign="top" width="150"><nobr>IP-Adress&nbsp;&nbsp;&nbsp;&nbsp;</nobr></td>
        <td valign="top" width="120"><nobr>Infos</nobr></td>
        <td valign="top" width="1" style="font-weight:bold"><nobr><a href="default.aspx?p=encoder&dt=<%=Server.UrlEncode(selectedFirstDate.AddDays(-7).ToString()) %>">&lt;&lt;</a></nobr></td>
    ...
    <% foreach (SAdevice device in encoders)
       { %>
    <tr>
        <td style="background-color:#e2e2e2"><nobr><%if (device.IP != "") { %><a href="http://<%=device.IP %>" target="_blank"><img src="picts/urlicon.gif" border="0" /></a>&nbsp;&nbsp;&nbsp;<% } %><a href="javascript:void(0)" <% if (device.Description != "") { %>onmouseover="showWMTT('<%=device.Description.Replace("\"", "&quot;").Replace("'", "\\'") %>')" onmouseout="hideWMTT();"<% } %> onclick="window.open('createSAdevice.aspx?id=<%=device.ID.ToString() %>','SAadmin','width=400,height=400,top=50,left=50,locationbar=no,menubar=no,resizable=0',true);"><%=device.Name%></a>&nbsp;&nbsp;&nbsp;<img src="picts/empty.gif" width="5" height="10" /></span></nobr></td>
        <td style="background-color:#e2e2e2"><%if (device.IP != "") { %><%=device.IP%> <% } %> </td> 
