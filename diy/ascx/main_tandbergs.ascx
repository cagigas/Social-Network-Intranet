...
<table width="100%" cellpadding="2" cellspacing="0" border="0">
    <tr style="background-color:#fbfbfb">
		...
		<td valign="top" width="1"><nobr>IP&nbsp;&nbsp;&nbsp;&nbsp;</nobr></td>
    ...
    <tr>
        ...
        <td style="background-color:#e2e2e2" ><%if (device.IP != "" && device.IP.Substring(0, 3).ToString() == "193") { %><%=device.IP %><% } %>&nbsp;&nbsp;</td>
    ...
    <tr>
        <td colspan="13"><img src="picts/empty.gif" border="0" width="10" height="1" style="display:block" /></td>
    </tr>
    <% } %>
    <tr>
        <td colspan="13"><img src="picts/empty.gif" border="0" width="10" height="10" style="display:block" /></td>
    </tr>
    <tr>
        <td colspan="13" style="font-weight:bold">
            Hybrids
    ...
    <tr>
        <td style="background-color:#e2e2e2"><nobr><%if (device.IP != "") { %><a href="http://<%=device.IP %>" target="_blank"><img src="picts/urlicon.gif" border="0" /></a>&nbsp;&nbsp;&nbsp;<% } %><a href="javascript:void(0)" onmouseover="showWMTT('<%=device.Description.Replace("\"", "&quot;").Replace("'", "\\'") %>')" onmouseout="hideWMTT();" onclick="window.open('createSAdevice.aspx?id=<%=device.ID.ToString() %>','SAadmin','width=400,height=400,top=50,left=50,locationbar=no,menubar=no,resizable=0',true);"><%=device.Name%></a></nobr></td>
        <td style="background-color:#e2e2e2" colspan="2"><% if (device.IP != "") { %><a href="http://<%=device.IP %>" target="_blank">Status</a><% } %></td>
        <td style="background-color:#e2e2e2" colspan="2"><nobr><%=device.Phone%></nobr></td>
        <% int counter = 0;
           for (int a = 0; a <= 6; a++)
           {
